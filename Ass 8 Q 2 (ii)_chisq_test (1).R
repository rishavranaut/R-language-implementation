# _______ Q2 (ii)._________.lindeley dist.
library(nleqslv)
...
chisquare_test <- function(k,t,num_samples){
  ...
  for (i in 1:num_samples) {
    ...
    sample[i] <- nleqslv( 1.2,f)$x      # using $x to retrieve root only 
  ...
  for ( j in 1:k){
    ...
    e= num_samples* ( (1 - ((t + 1 + t * mini) / (t + 1)) * exp(-t * mini)) -(1 - ((t + 1 + t * m1) / (t + 1)) * exp(-t * m1 ) ) )
...
#for the samples and t for generating samples
chisquare_test(5,1,1000)
