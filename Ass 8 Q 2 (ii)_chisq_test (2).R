...
# _______ Q3 (i)._________ normal dist.

# Hypothesis
# H0 : sample fits the data
# Ha: not .

chisquare_test <- function(k,mew,sigma, num_samples){
  
  #generating sample space
  sample <-c()
  for (i in 1:num_samples) {
    sample[i] <- rnorm(1,mew,sigma)
  }
  ...
  for ( j in 1:k){
    ...
    e=num_samples * ( ( pnorm ( mini, mew , sigma)) - pnorm( m1, mew ,sigma ) )
...
#for the samples ,mew ,sigma for generating samples
